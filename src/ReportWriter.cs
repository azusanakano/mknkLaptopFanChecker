...
namespace Mknk.LaptopFanChecker
{
    public static class ReportWriter
    {
        ...
        private static void WriteCsv(string path, IList<TelemetrySample> samples)
        {
            ...
            {
                writer.WriteLine("timestamp,elapsed_seconds,phase,cpu_temperature_c,cpu_load_percent,fan_rpm,fan_control_percent,fan_sensor_available,temperature_sensor,fan_sensor,sensor_source");
            ...
            string csvPath)
        {
            ...
            html.Append("<h1>ネコシステム社 ノートPC CPUファンチェッカー</h1><div class=\"sub\">検査レポート / ")
            ...
            html.Append("<div class=\"section\"><h2>測定ログ</h2><p><small>CSV: ")
                .Append(H(Path.GetFileName(csvPath))).Append("</small></p><div class=\"scroll\"><table><thead><tr><th>時刻</th><th>工程</th><th>経過</th><th>温度</th><th>CPU負荷</th><th>Fan</th><th>センサー</th></tr></thead><tbody>");
            foreach (TelemetrySample sample in samples)
            {
            ...
            html.Append("<div class=\"section notice\"><b>注意：</b>本ツールは故障の可能性を絞る検査補助です。RPM非公開機では温度挙動から推定します。メーカー診断、目視、排気・異音確認を置き換えません。ファン制御値やBIOS設定は変更しません。</div>");
            html.Append("<p><small>mknkLaptopFanChecker ").Append(H(AppInfo.Version))
        ...
        {
            string result = String.IsNullOrWhiteSpace(value) ? "Laptop" : value;
